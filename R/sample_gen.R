...
#'
#' @returns
#' Retorna uma \code{lista} contendo dois elementos: \code{fun}, onde cada linha
#' é um elemento da amostra gerada, e \code{y}, os quais são os pesos
#' utilizados.
#'
#' @examples
...
#' plot(doppler, type='l', main='Doppler')
#' par(mfrow=c(1,1))
#'
...
sample_gen <- function(fun_comp, snr, n = 10, signal = 7,
  ...
  if (erro == 'normal') {
    ...
    fun_agr <- t(y) %*% fun_comp + rgamma(n*ncol(fun_comp), shape=beta, rate=lambda)
  }
