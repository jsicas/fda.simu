...
L <- 5                 # quantidade de iterações
theta_1 <- rep(-30, n)  # chute inicial
S_1 <- diag(n)         # chute inicial
alpha <- 0.8
tau <- 2
lambda <- 40  # parâmetro gamma
beta <- 10  # parâmetro gamma
d <- wd(f_test(n)$bumps + dgamma(n, shape=beta, rate=lambda),
...
# post_gamma(theta_star, d, alpha, beta, tau, lambda)
